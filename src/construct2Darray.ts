...
interface Construct2D {
  (original: number[], m: number, n: number): number[][];
}

const construct2Darrary: Construct2D = (original, m, n) => {
  const converted: number[][] = [];
  // check the 2D array construction is possible or not
  if (original.length !== m * n) return converted;

  // loop through the original array m times to construct a 2D array
  for (let i = 0; i < m; i++) {
    const row: number[] = original.slice(i * n, (i + 1) * n);
    converted.push(row);
  }
